...
int n_Para;
int Is_Para_Fixed[N_PARA_MAX], Is_Dih_Fixed[N_MAX_DIH];
...
void Output_Rotamer_QM_MM(void)
{
	int i;
	FILE *fOut;
	double Emin_QM=1.0E300, Emin_MM=1.0E300;
...
//	CalObjectiveFunction(Para_List, 1);
	...
	for(i=0; i<nRotamer; i++)	{
		if(E_Rotamer_QM[i] < Emin_QM)	{
			Emin_QM = E_Rotamer_QM[i];
		}
		if(E_Rotamer_MM[i] < Emin_MM)	{
			Emin_MM = E_Rotamer_MM[i];
		}
	}

	fOut = fopen("org-rotamer-E.dat", "w");
	for(i=0; i<nRotamer; i++)	{
		fprintf(fOut, "%4d %.5lf %.5lf\n", i+1, E_Rotamer_QM[i]-Emin_QM, E_Rotamer_MM[i]-Emin_MM);
	}
	fclose(fOut);
}
...
double Optimize_Torsion_Parameters(void)
{
	...
	for(i=0; i<n_Phi; i++)	{
		...
		if(Is_Dih_Fixed[i])	{
			if(ProgID == 0)	{
				fprintf(fFitting, "%d dihedral parameters are fixed.\n", i+1);
				fflush(fFitting);
			}

			for(j=1; j<10; j++)	{	// the phase is fixed
				Is_Para_Fixed[Pos+j] = 1;
			}
		}
	}
...
char szName_Run_Log[]="org-1d-rotamer-log.txt";
