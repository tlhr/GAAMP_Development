...
void Enumerate_Dihedrals(int IdxDih)	// depth
{
	...
	if(IdxDih < n_Phi)	{
		...
		sprintf(szName, "opt-%d.crd", Counter);
		fIn = fopen(szName, "r");
...
//		Mol.SavePdb(szNameIni);
		Do_Geometry_Optimization(&Mol);
		Mol.WriteMyCRDFile(szName);	// high precision
...
int main(int argc, char **argv)
{
	...
	char szOutput[]="output.out", *szEnv;

	if(argc != 2)	{
		printf("Usage: qm-rotamer-scan-large rmsd_cutoff\n");
		exit(1);
	}
...
void Gen_QM_Rotamer_Data(void)
{
	FILE *fIn;
	int i, nAtom;
	char szName[256], szCmd[256], szOutput[]="output.out";
	...
	for(i=0; i<nCluster; i++)	{
		chdir(szCurDir);
		sprintf(szName, "qm-ini-%d.pdb", i+1);
		...
		strcpy(szDirList[i], szNewDir);
		strcpy(szInputList[i], szGjfFile);
		sprintf(szOutputList[i], "%s/%s", szNewDir, szOutput);
		nJob++;
	}
...
void Count_Intramolecular_HBond(double x[], double y[], double z[], int nAtom, int& n_HBond, int& n_Far_HBond)
{
	...
	for(i=0; i<nAcceptor; i++)	{
		...
		for(j=0; j<nHDonor; j++)	{
			...
			if( (r < r_Cut) && (dist[Atom_1][Atom_2] > 2) )	{
//				printf("r(%d, %d) = %.2lf %2d ", Atom_1+1, Atom_2+1, r, dist[Atom_1][Atom_2]);
