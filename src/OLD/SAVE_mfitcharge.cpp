...
//	pMol_ESP->SavePdb("before.pdb");
	...
	pMol_ESP->FullGeometryOptimization_LBFGS(1);	//let the drudes relax
	pMol_ESP->Cal_Dipole(MM_Dipole_x, MM_Dipole_y, MM_Dipole_z, MM_Dipole);
...
int CFitting::IsThereFarDrudes(double& R_SQ_MAX)
{
	...
	for(int Check=0; Check<30; Check++)	{
		...
		pMol_ESP->FullGeometryOptimization_LBFGS(0);	//let all particles relax. Only optimize the geometry of the molecule
...
//	pMol_ESP->SavePdb("2.pdb");
	...
	if(Target_E_Int_Water)	{	// to do fully geometry optimization
		...
		pMol_ESP->nAtom--;
		pMol_ESP->FullGeometryOptimization_LBFGS(0);
		pMol_ESP->nAtom++;
...
double CFitting::Cal_Chi_SQ_E_Int_Dimer(void)
{
	...
	for(iConf=0; iConf<N_CONF_DIMER; iConf++)	{
		...
		pNMA_Single->FullGeometryOptimization_LBFGS(1);	//let the drudes relax
		...
		pNMA_Dimer->FullGeometryOptimization_LBFGS(1);	//let the drudes relax
...
//	for(iConf=400; iConf<nConf_Mol_Ar; iConf++)	{
		...
		pMol_Ar->FullGeometryOptimization_LBFGS(1);	//let the drudes relax
...
//	pMol_Wat->SavePdb("tmp.pdb");
	...
	pMol_Wat->FullGeometryOptimization_LBFGS(1);	//let the drudes relax
			 ...
			 double& x_D, double& y_D, double& z_D)
{
	...
	vz_y = y_C - y_B;
	vz_z = z_C - z_B;
	Normalize(vz_x, vz_y, vz_z);	

	BA_x = x_A - x_B;
	BA_y = y_A - y_B;
	BA_z = z_A - z_B;
	Normalize(BA_x, BA_y, BA_z);	

	vy_x = vz_y*BA_z - vz_z*BA_y;
	vy_y = vz_z*BA_x - vz_x*BA_z;
	vy_z = vz_x*BA_y - vz_y*BA_x;
	Normalize(vy_x, vy_y, vy_z);
...
double COptimizeWaterDimer::CalObjectiveFunction_Mol_Wat_Dimer_E_Int(void)	// only used for the optimization in MM
{
	...
	pDimer->FullGeometryOptimization_LBFGS(1);	// the total energy of molecule and water
...
void COptimizeWaterDimer::Rotate_Dummy_Atoms(int theta)
{
	if(IsAcceptor)	{	// acceptor
		...
		Normalize(v_x_X1_Acceptor, v_y_X1_Acceptor, v_z_X1_Acceptor);
		...
		Normalize(v_x_X1_H, v_y_X1_H, v_z_X1_H);
...
void COptimizeWaterDimer::Generate_Dummy_Atoms_Acceptor_1(void)
{
	...
	Normalize(v_x_Acceptor_X2, v_y_Acceptor_X2, v_z_Acceptor_X2);
...
void COptimizeWaterDimer::Generate_Dummy_Atoms_Acceptor_2(void)
{
	...
	Normalize(vx_Acceptor_A, vy_Acceptor_A, vz_Acceptor_A);

	vx_Acceptor_B = x[i_B] - x[i_O];
	vy_Acceptor_B = y[i_B] - y[i_O];
	vz_Acceptor_B = z[i_B] - z[i_O];
	Normalize(vx_Acceptor_B, vy_Acceptor_B, vz_Acceptor_B);
...
void COptimizeWaterDimer::Generate_Dummy_Atoms_Acceptor_3(void)
{
	...
	Normalize(vx_Acceptor_C, vy_Acceptor_C, vz_Acceptor_C);
	...
	r_Proj = Normalize(v_x_X1_Acceptor, v_y_X1_Acceptor, v_z_X1_Acceptor);
...
void COptimizeWaterDimer::Generate_Dummy_Atoms_Donor(void)
{
	...
	Normalize(v_x_H_X2, v_y_H_X2, v_z_H_X2);
