...
double Normalize_Local(double& vect_x, double& vect_y, double& vect_z);
...
//eliot	int bFarDrude_List[MAX_N_PROC];
//eliot	double R_SQ_MAX_List[MAX_N_PROC];
//eliot	MPI_Gather(&bFarDrude,1,MPI_INT,bFarDrude_List,1,MPI_INT,MY_MPI_ROOT,MPI_COMM_WORLD);
//eliot	MPI_Gather(&R_SQ_MAX,1,MPI_DOUBLE,R_SQ_MAX_List,1,MPI_DOUBLE,MY_MPI_ROOT,MPI_COMM_WORLD);
//eliot	if(ProgID == MY_MPI_ROOT)	{
/*eliot
		bFarDrude = 0;
		for(i=0; i<nProc; i++)	{
...
//eliot	}
//eliot	MPI_Bcast(&bFarDrude, 1, MPI_INT, MY_MPI_ROOT,MPI_COMM_WORLD);
//eliot	MPI_Bcast(&R_SQ_MAX, 1, MPI_DOUBLE, MY_MPI_ROOT,MPI_COMM_WORLD);
...
double CFitting::Cal_Chi_SQ_Int_Energy_Mol_Water(void)
{
	...
	for(iConf=0; iConf<nConf_Mol_Wat; iConf++)	{
		...
		pMol_Wat->TranslateLastSegment();	// move the last segment (the other molecule) far
		pMol_Wat->Geo_Opt_Drude_Only = 1;	//only drude will be relaxed
		pMol_Wat->FullGeometryOptimization_LBFGS();	//let the drudes relax
...
//		else if(strncmp(szReadConf_File[i], "FILE_CRD", 8) == 0)	{
			 ...
			 double& x_D, double& y_D, double& z_D)
{
	...
	vz_y = y_C - y_B;
	vz_z = z_C - z_B;
	Normalize_Local(vz_x, vz_y, vz_z);	

	BA_x = x_A - x_B;
	BA_y = y_A - y_B;
	BA_z = z_A - z_B;
	Normalize_Local(BA_x, BA_y, BA_z);	

	vy_x = vz_y*BA_z - vz_z*BA_y;
	vy_y = vz_z*BA_x - vz_x*BA_z;
	vy_z = vz_x*BA_y - vz_y*BA_x;
	Normalize_Local(vy_x, vy_y, vy_z);
...
void COptimizeWaterDimer::Rotate_Dummy_Atoms(int theta)
{
	if(IsAcceptor)	{	// acceptor
		...
		Normalize_Local(v_x_X1_H, v_y_X1_H, v_z_X1_H);
...
int COptimizeWaterDimer::Is_Three_Atoms_Colinear(int ia, int ib, int ic)
{
	...
	Normalize_Local(v1_x, v1_y, v1_z);
	Normalize_Local(v2_x, v2_y, v2_z);
...
void COptimizeWaterDimer::Generate_Dummy_Atoms_Acceptor_1(void)
{
	...
	Normalize_Local(v_x_Acceptor_X2, v_y_Acceptor_X2, v_z_Acceptor_X2);
...
void COptimizeWaterDimer::Generate_Dummy_Atoms_Acceptor_2(void)
{
	...
	Normalize_Local(vx_Acceptor_A, vy_Acceptor_A, vz_Acceptor_A);

	vx_Acceptor_B = x[i_B] - x[i_O];
	vy_Acceptor_B = y[i_B] - y[i_O];
	vz_Acceptor_B = z[i_B] - z[i_O];
	Normalize_Local(vx_Acceptor_B, vy_Acceptor_B, vz_Acceptor_B);
...
void COptimizeWaterDimer::Generate_Dummy_Atoms_Acceptor_3(void)
{
	...
	Normalize_Local(vx_Acceptor_C, vy_Acceptor_C, vz_Acceptor_C);
	...
	r_Proj = Normalize_Local(v_x_X1_Acceptor, v_y_X1_Acceptor, v_z_X1_Acceptor);
...
void COptimizeWaterDimer::Generate_Dummy_Atoms_Donor(void)
{
	...
	Normalize_Local(v_x_H_X2, v_y_H_X2, v_z_H_X2);
