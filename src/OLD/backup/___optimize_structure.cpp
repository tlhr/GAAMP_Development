...
#define LOW_BOUND_CG	(-1.25)	// for heavy atoms
...
//				nAtm_From_Name = FindAtomsinMol(ItemList[Idx_Item], IdxList);
			...
			if(nAtm_in_Neutral_Frag > 1)	{
				...
				if(i>=nAtm_in_Neutral_Frag)	{	// fail to find a heavy atom within the fragment. Something wrong?
...
//			if(AtomType[IdxList_Frag[i]].NotFitCharge == 0)	{
		...
		if(i>=nAtm_in_Neutral_Frag)	{	// fail to find a valid atom within the molecule. Something wrong?
...
double Callback_Eval_Gradient(unsigned n, const double *x, double *grad, void *my_func_data)
{
	...
	if(grad)	{
		...
		if(fitting.Chi_SQ < Chi_SQ_Min)	{
			...
			if(FailCount > 20)      {       // cannot converge due to limited accuracy of numerical gradient
				printf("Cannot fully converge the charges due to the limited accuracy of numerical gradients\nIt is assumed to be correct.\nPlease verify\n");
...
//	fprintf(fOut, "\nCG_Sum = %lf\n", CG_Sum);
	...
	for(i=0; i<nAtom; i++)	{
		if(pMol->IsDrude[i])	{	//skip drude
			...
			fprintf(fOut, "ATOM %-6s %-8s ", pMol->AtomName[i], pMol->ChemName[i]);
			fprintf(fFinal, "ATOM %-6s %-8s ", pMol->AtomName[i], pMol->ChemName[i]);
			if( ( (i+1) < nAtom) && (pMol->IsDrude[i+1]) )	{
				fprintf(fOut, "%6.3lf  ALPHA %6.3lf  THOLE %5.3lf\n", q[i], alpha[i], thole[i]);
...
// ELIOT no longer in use for new version of nlopt ????
