...
int main(int argc, char *argv[])
{
	...
	for(i=0; i<n_Atom_In_Mol; i++)	{
		...
		if(ToGenerate_Water)	{
			ActiveAtom = i;
			if( (Idx_Acceptor+1) != AcceptorSelect )	{	// only run for one selected acceptor
				Idx_Acceptor++;
				continue;
			}
...
//					printf("E_Shifted = %lf\n", E_Min_MM_Shifted);
			...
			if( (Bond_Count[i] <= 2) && (E_Min_MM > -2.0) )	{	// to generate water along a perp orientation
...
//							GenerateWater();
			...
			r_Emin_QM = Cal_Dist_Two_Atoms(ActiveAtom, n_Atom_In_Mol+1);	// donor - H1 / H2
			...
			fprintf(fOut, "#Energy    %3d acceptor %2d atom %3s QM ", Idx_Acceptor+1, ActiveAtom+1, szElemName[ActiveAtom]);
			fprintf(fOut, "%12.6lf %9.4lf ", E_Min_QM, r_Emin_QM);
			fprintf(fOut, "\n");

			fprintf(fOut, "Acceptor %5d %12.6lf %9.4lf \n", ActiveAtom+1, E_Min_QM, r_Emin_QM);
...
//	Generate_Gaussian_Input();
	...
	if(argc == 2)	{
		fOut = fopen("count-acceptor.txt", "w");
		fprintf(fOut, "%d\n", Idx_Acceptor);
		fclose(fOut);
	}
...
double Optimize_Water_Pose_QM(void)
{
	...
	char szCmd[256], szName_Result[256]="opt-output.out", szLine[256], *ReadLine, szStr_Energy[256], szAcceptor[256];
	double E_Min;
