...
char szQMParaFile[]="/home/eliotblg/progs/GAAMP-1.0.0/scripts/QM-para.txt";
...
char szMyPath[]="/home/eliotblg/progs/GAAMP-1.0.0/scripts/mypath.txt";
...
int Is_N[MAX_ATOM_MOL], Is_O[MAX_ATOM_MOL], Is_F[MAX_ATOM_MOL];
int Bond_Count[MAX_ATOM_MOL], BondList[MAX_ATOM_MOL][4], BondWithH[MAX_ATOM_MOL];
...
double Optimize_Water_Pose_QM(void);
...
int main(int argc, char *argv[])
{
	...
	for(i=0; i<n_Atom_In_Mol; i++)	{
		...
		if(ToGenerate_Water)	{
			...
			if( E_Min_MM > -2.0 )	{	// to generate water along a perp orientation
				for(theta=-30; theta<=30; theta+=60)	{	// only -45 and 45
			...
			RestoreDummyAtoms();
			fprintf(fFile_Run_Log, "mol_H_donor.cpp> %2d H donor: %2d atom %3s MM, E_min = %7.4lf r_min = %7.4lf\n", Idx_Donor+1, i+1, szElemName[i], E_Min_MM, r_Emin_MM);
			...
			sprintf(szWorkDir, "%s/dat-donor-%d-%d/QM", szCurDir, Idx_Donor+1, ActiveAtom+1);
			sprintf(szCmd, "mkdir %s", szWorkDir);
			system(szCmd);
			chdir(szWorkDir);
			
			E_Min_QM = (Optimize_Water_Pose_QM()-E_Dimer_Far)*HARTREE_To_KCAL;
			...
			r_Emin_QM = Cal_Dist_Two_Atoms(ActiveHAtom, n_Atom_In_Mol);	// H - OH2
			...
			fprintf(fFile_Run_Log, "mol_H_donor.cpp> %2d H donor: %2d atom %3s QM, E_min = %7.4lf r_min = %7.4lf\n", Idx_Donor+1, i+1, szElemName[i], E_Min_QM, r_Emin_QM);
			fflush(fFile_Run_Log);

			chdir(szCurDir);
			fOut = fopen(szMolWat_E_Int, "a+");

#ifndef _WIN32
			lockf(fileno(fOut),F_LOCK,0L);
#endif
			fseek(fOut, SEEK_END, 0);
			fprintf(fOut, "#Energy    %3d donor %2d atom %3s QM ", Idx_Donor+1, ActiveAtom+1, szElemName[ActiveAtom]);
			fprintf(fOut, "%12.6lf %9.4lf ", E_Min_QM, r_Emin_QM);
			fprintf(fOut, "\n");

			fprintf(fOut, "Donor %5d %12.6lf %9.4lf \n", ActiveAtom+1, E_Min_QM, r_Emin_QM);
			fprintf(fOut, "Saved_MM_Parameters %3d %22.16lf %22.16lf\n", 
				theta_Save, Para_Save_for_QM[0], Para_Save_for_QM[1]);
			fclose(fOut);
...
void GetBondList(int Idx)
{
	...
	for(i=0; i<nBond; i++)	{
		...
		if(pBondList[iPos] == Idx)	{
			...
			Bond_Count[Idx]++;
			if( (BondWithH[Idx] < 0) && (strcmp(szElemName[Atom_2], "H")==0))	{
				BondWithH[Idx] = Atom_2;
			}
		}
...
{
	FILE *fOut, *fIn;
	int i, ReadItem, iTmp, AtomNum;
	char szCmd[256], szName_Result[256]="opt-output.out", szLine[256], *ReadLine, szStr_Energy[256], szDonor[256], szDonorH[256];
	double E_Min;
