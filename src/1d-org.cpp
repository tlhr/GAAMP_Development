...
double Optimize_Torsion_Parameters(int IdxPhi);
...
//	Optimize_Torsion_Parameters(Idx);

	sprintf(szName, "org-1d-%d.dat", Idx+1);
	fOut = fopen(szName, "w");
	for(i=0; i<nScan; i++)	{
		fprintf(fOut, "%.1lf %lf %lf\n", Phi_Scaned[i], E_QM_Scaned[i], E_MM_Scaned[i]);
	}
	fclose(fOut);
...
/*
	...
	for(i=0; i<(N_TOR_PARA-1); i++)	{
		fprintf(fOut, "%.12lf ", All_Para_Save[0][i]);
	}
	fprintf(fOut, "\n");
	fclose(fOut);
*/
...
//	memset(Mol_ESP.Para_phi[IdxDihSelect[Idx]]+1, 0, sizeof(double)*6);
	...
	sprintf(szName, "org-mm-tor-1D-idx-%d.dat", Idx+1);	// the one with optimized geometry in MM force field
	fOut = fopen(szName, "w");
...
//	pMol->SavePdb("opt-free.pdb");
	...
	for(i=0; i<pMol->nBond_Fixed; i++)	{
		...
		nlopt_add_equality_constraint(opt_AugLag, Geo_Constraint_Bond, &(pMol->Geo_Fix_r0[i]), 2.0E-6);
	}

	for(i=0; i<pMol->nAngle_Fixed; i++)	{
		pMol->Geo_Fix_theta0[i].pMol = pMol;
		nlopt_add_equality_constraint(opt_AugLag, Geo_Constraint_Angle, &(pMol->Geo_Fix_theta0[i]), 2.0E-6);
	}

	for(i=0; i<pMol->nPhi_Fixed; i++)	{
		pMol->Geo_Fix_phi0[i].pMol = pMol;
		nlopt_add_equality_constraint(opt_AugLag, Geo_Constraint_Dihedral, &(pMol->Geo_Fix_phi0[i]), 2.0E-6);
	}
...
char szName_Run_Log[]="1d-org-log.txt";
