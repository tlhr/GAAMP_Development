...
int ParticleIndex[MAX_ATOM_LOCAL];
void BuildParticleIndex(CMol *pMol);
...
void Rewrite_Prm_File(int Flag, char szAddTxt[], char szChemName[][16]);
int Find_Dihedral_Entries_By_Name(char szChemName[][16], int List[]);
void Generate_New_Prm_Text(char szText[], char szChemName[][16], int Phi_Idx);
...
double E_Correct;	// the correct energy, used to check whether the new files are correct or not

FILE *fFile_Run_Log, *fDihedral;
...
void Update_Xpsf_File_with_New_Atom_Type(int ActiveAtom_1, int ActiveAtom_4)
{
	...
	fIn = fopen("drude-mol.xpsf", "r");
	if(fIn == NULL)	{
		Quit_With_Error_Msg("Fail to open file: drude-mol.xpsf in Update_Xpsf_File_with_New_Atom_Type().\nQuit\n");
	}
	nLine = 0;
	while(1)	{
		if(feof(fIn))	{
...
int main(void)
{
	...
	Read_Atom_Type_RTF();
	ForceField.ReadForceField("drude-mol.prm");	// read the force field. Parameter file. It is fine if the file includes rtf info. 
	
	Mol.drude_ReadPSF("drude-mol.xpsf", 0, 1);	//the first parameter is the xpsf file name; the second parameter is 0 (only one molecule in xpsf) or 1 (two molecules in xpsf). 
	BuildParticleIndex(&Mol);
...
void Generate_A_New_ChemName(int iAtom, char szOldChemName[])
{
	...
	for(i=0; i<PoolSize; i++)	{
		CharEnd = CharPool[i];
		sprintf(NewChemName, "%s_%c", BaseName, CharEnd);
...
void Update_rtf_prm_xpsf(int Idx)
{
	...
	char ChemName[4][16], szTxtAdd[1024], szChemPara[2][12], szBuff[256];
	double Para_List[6];
...
#define MAX_LINE_PRM	(8192)
void Rewrite_Prm_File(int Flag, char szAddTxt[], char szChemName[][16])
{
	FILE *fIn, *fOut;
	char szPrmTxt[MAX_LINE_PRM][256], szName[]="drude-mol.prm", *ReadLine, szChemRead[4][256];
	int i, nLine, To_Output[MAX_LINE_PRM], Idx_Line_Dihedral, Idx_Last_Entry_Dihedral, Idx_Line_Delete, ReadItem, Order;
	double k_Dih, Phi0; 

	fIn = fopen(szName, "r");
	if(fIn == NULL)	{
		Quit_With_Error_Msg("Fail to open drude-mol.prm for read in Rewrite_Prm_File().\nQuit\n");
	}
	nLine=0;
	while(1)	{
		if(feof(fIn))	{
	...
	if(Idx_Line_Dihedral < 0)	{
		Quit_With_Error_Msg("Fail to find the tag DIHEDRALS in drude-mol.prm.\nQuit\n");
	}
	...
	if(Idx_Last_Entry_Dihedral < 0)	{
		Quit_With_Error_Msg("Fail to find the last entry of DIHEDRALS in drude-mol.prm.\nQuit\n");
	}
...
{
	FILE *fIn;
	char szPrmTxt[MAX_LINE_PRM][256], szName[]="drude-mol.prm", *ReadLine, szChemRead[4][256], ErrorMsg[256];
		...
		if(i==Idx_Last_Entry_Bond)	{
			...
			for(j=0; j<nDihedral_Add; j++)	{
				if(To_Add_Dihedral[j])	{
					...
					fprintf(fOut, "%-8s %-8s %-8s %-7s  %.15lf %3.0lf %9.1lf\n", 
...
void BackupCurrent_Torsion_Parameters(void)
{
	...
	Mol.Geo_Opt_Drude_Only = 0;	//only drude will be relaxed
	Mol.BuildActiveEnergyTermList();

	Mol.ReadCRD_RealAtom("mol-opt.crd");	// read the coordinate file in a non-polarizable FF
	printf("----------------------------------------\nUsing the original prm/xpsf files, before geometry optimization\n");
	Mol.Cal_E(1);
	printf("----------------------------------------\n\n\n");

	printf("----------------------------------------\nUsing the original prm/xpsf files\n");
	Mol.Geo_Opt_Drude_Only = 1;	//only drude will be relaxed
	Mol.BuildActiveEnergyTermList();
...
int Read_Soft_DihedralList(void)
{
	...
	while(1)	{
		...
		if(ReadItem == 4)	{
			DihList[n_Phi][0] = Mol.RealAtom_List[DihList[n_Phi][0]-1];
			DihList[n_Phi][1] = Mol.RealAtom_List[DihList[n_Phi][1]-1];
			DihList[n_Phi][2] = Mol.RealAtom_List[DihList[n_Phi][2]-1];
			DihList[n_Phi][3] = Mol.RealAtom_List[DihList[n_Phi][3]-1];
...
int IsValid_Updated_Prm(void)
{
	...
	for(Idx=0; Idx<n_Dihedral; Idx++)	{
		...
		for(i=1; i<=6; i++)	{
			if( fabs(Dih_Para_k[Idx][i] - Mol.Para_k_Dih[Idx][i]) > 1.0E-6 )	{	// something wrong?
...
//			sprintf(szLine, "%-7s %-7s %-7s %-7s    %.5lf         %d    %6.1lf\n", 
//				szChemName[0], szChemName[1], szChemName[2], szChemName[3], Dih_Para_k[Phi_Idx][i], i, Dih_Para_Phi[Phi_Idx][i]*radian);
...
void To_Add_New_Atom_Type_Rtf(int ActiveAtom_1, char szOldChem_1[], int ActiveAtom_4, char szOldChem_4[])
{
	...
	IdxAtom_1 = ParticleIndex[ActiveAtom_1];
	if(ActiveAtom_4 >= 0)	{
		IdxAtom_4 = ParticleIndex[ActiveAtom_4];
	}
	else	{
		IdxAtom_4 = -1;
	}
	...
	for(; i<nLine; i++)	{
		...
		if(strncmp(szRtfTxt[i], "ATOM", 4)==0)	{
			...
			if(AtomCount == IdxAtom_1)	{	// the atom to be changed
				...
				if(Mol.mass[ActiveAtom_1] > 1.5)	{	// A heavy atom, not a hydrogen atom.
					sprintf(szRtfTxt[i], "ATOM %-7s %-8s %7.3lf  ALPHA %6.3lf  THOLE %5.3lf\n", 
						Mol.AtomName[ActiveAtom_1], Mol.ChemName[ActiveAtom_1], Mol.CG[ActiveAtom_1]+Mol.CG[ActiveAtom_1+1], Mol.alpha[ActiveAtom_1], Mol.thole[ActiveAtom_1]);
				}
				else	{
					sprintf(szRtfTxt[i], "ATOM %-7s %-8s %7.3lf\n", Mol.AtomName[ActiveAtom_1], Mol.ChemName[ActiveAtom_1], Mol.CG[ActiveAtom_1]);
				}
...
//				break;
			...
			else if(AtomCount == IdxAtom_4)	{
				strcpy(szLineSave, szRtfTxt[i]);
				if(Mol.mass[ActiveAtom_4] > 1.5)	{	// A heavy atom, not a hydrogen atom.
					sprintf(szRtfTxt[i], "ATOM %-7s %-8s %7.3lf  ALPHA %6.3lf  THOLE %5.3lf\n", 
						Mol.AtomName[ActiveAtom_4], Mol.ChemName[ActiveAtom_4], Mol.CG[ActiveAtom_4]+Mol.CG[ActiveAtom_4+1], Mol.alpha[ActiveAtom_4], Mol.thole[ActiveAtom_4]);
				}
				else	{
...
//				break;
	...
	for(i=0; i<nLine; i++)	{
		...
		if(i==LineLastMassMol)	{	// add one line for the new atom type
			if(Mol.mass[ActiveAtom_1] > 1.5)	{	// a heavy atom
				fprintf(fOut, "MASS %5d %-8s %10.6lf\n", AtomeTypeCount+1, Mol.ChemName[ActiveAtom_1], Mol.mass[ActiveAtom_1]+Mol.mass[ActiveAtom_1+1]);
			}
			else	{
				fprintf(fOut, "MASS %5d %-8s %10.6lf\n", AtomeTypeCount+1, Mol.ChemName[ActiveAtom_1], Mol.mass[ActiveAtom_1]);
			}
			if(ActiveAtom_4 >= 0)	{
				if(Mol.mass[ActiveAtom_4] > 1.5)	{	// a heavy atom
					fprintf(fOut, "MASS %5d %-8s %10.6lf\n", AtomeTypeCount+2, Mol.ChemName[ActiveAtom_4], Mol.mass[ActiveAtom_4]+Mol.mass[ActiveAtom_4+1]);
				}
				else	{	// H
...
void Determine_Bond_Angle_Dihedral_Improper_Add_1(int ActiveAtom, int Active_Phi)
{
	...
	for(i=0; i<nBond; i++)	{
		...
		if( (ActiveAtom == ia) || (ActiveAtom == ib) )	{
			if( Mol.IsDrude[ia] || Mol.IsDrude[ib] )	{
				continue;
			}
	...
	for(i=0; i<nDihedral; i++)	{
		...
		if(i == Active_Phi)	{	// to add all parameters for this soft dihedral
			...
			if(LocalCount == 0)	{	// We need to add a dummy entry with k=0. We need to find the entry in rtf file
				...
				To_Add_Dihedral[nDihedral_Add] = 1;
				nDihedral_Add++;
//				Quit_With_Error_Msg("LocalCount == 0\nCode is not completed yet in Determine_Bond_Angle_Dihedral_Improper_Add().\nQuit\n\n");
			}
		}
	}
...
void BuildParticleIndex(CMol *pMol)
{
	...
	for(i=0; i<nAtom; i++)	{
		if(pMol->IsDrude[i])	{
			ParticleIndex[i] = 0xBFFFFFFF;	// an invalid index
		}
		else	{
			Count++;
			ParticleIndex[i] = Count;
		}
	}
}
