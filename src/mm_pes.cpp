...
#include <math.h>
#include <time.h>
//#include <mpi.h> eliot : no mpi for the moment
#include "ff.h"
#include "nlopt.h"
...
void Enumerate_Dihedrals(int IdxDih)	// depth
{
	...
	if(IdxDih < n_Phi)	{
		...
		sprintf(szName, "opt-%d.pdb", Counter);
		fIn = fopen(szName, "r");
...
/* eliot : commented out for the moment, might get back to it later
	MPI_Init(&argc, &argv);
