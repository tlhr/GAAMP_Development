...
double Optimize_Torsion_Parameters(void)
{
	...
	opt = nlopt_create(NLOPT_LD_LBFGS, n_Para);
...
//	Read_Parameters("saved-para.dat");
	...
	FailCount = 0;
	if (nlopt_optimize(opt, Para_List, &Chi_SQ) < 0) {
		printf("nlopt failed!\n");
	}
	else {
...
//	MPI_Comm_size(MPI_COMM_WORLD, &nProc);
	...
	Mol_ESP.drude_ReadPSF(szName_XPSF, 0, 1);	// to delete the last atom, test charge
	Read_Rtf_File();
	
	strcpy(Mol_ESP.MolName, Mol_ESP.ResName[0]);
	Mol_ESP.AssignForceFieldParameters(&ForceField);
//	Mol_ESP.Geo_Opt_Drude_Only = 1;
//	Mol_ESP.BuildActiveEnergyTermList();
...
int ReadConfFile(char szName[])
{
	...
	for(i=0; i<n_Line_Conf_File; i++)	{
		...
		else if(FindTags_As_First_String(szReadConf_File[i], "FILE_CRD"))	{
...
{
	FILE *fIn;
	char szLine[256], *ReadLine, szTmp[256];
	int ReadItem, i, nAtom, nRealAtom, *pRealAtom_List, Idx_Atom;
	...
	if(fIn == NULL)	{
		printf("Fail to open file: all-rotamer.dat\nQuit\n");
		exit(1);
	}
	nRotamer = 0;
			...
			if(ReadItem == 2)	{
				...
				if(strncmp(szLine, "Coordinate", 10)==0)	{
					for(i=0; i<nRealAtom; i++)	{
						...
						sscanf(szLine, "%lf %lf %lf", &(x_Rotamer[nRotamer][Idx_Atom]), &(y_Rotamer[nRotamer][Idx_Atom]), &(z_Rotamer[nRotamer][Idx_Atom]));
						if(Mol_ESP.mass[Idx_Atom] > 1.5)	{	// a heavy atom. To set up the coordinate for the drude particle
							x_Rotamer[nRotamer][Idx_Atom+1] = x_Rotamer[nRotamer][Idx_Atom];
							y_Rotamer[nRotamer][Idx_Atom+1] = y_Rotamer[nRotamer][Idx_Atom];
							z_Rotamer[nRotamer][Idx_Atom+1] = z_Rotamer[nRotamer][Idx_Atom];
						}
					}
...
void Read_1D_Scan_QM_Data(void)
{
	...
	FILE *fIn;
	char szName[256], *ReadLine, szLine[256], szTmp[256];
	double fTmp;
					...
					if(strncmp(szLine, "Coordinate", 10)==0)	{
						...
						for(j=0; j<nAtom; j++)	{
							...
							sscanf(szLine, "%lf %lf %lf", 
								&(x_Scan_list[i][nScan_List[i]][j]), 
								&(y_Scan_list[i][nScan_List[i]][j]), 
								&(z_Scan_list[i][nScan_List[i]][j])); 
						}
...
//		Mol_ESP.Restrain_All_Torsions(1);
		Mol_ESP.FullGeometryOptimization_LBFGS();
		Mol_ESP.Restrain_All_Torsions(0);
