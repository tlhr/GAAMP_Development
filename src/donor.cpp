...
//			ToGenerate_Water = 1;
//		}
                if ( Is_F[i] ) // generate with halogens for XBonds Eliot
                {
                  generateXbond = 1;
                }
...
void Make_List_H_Acceptor(void)
{
	...
	for(i=0; i<n_Atom_In_Mol; i++)	{
		...
		else if(strcmp(szElemName[i], "F")==0 || strcmp(szElemName[i], "CL")==0 || strcmp(szElemName[i], "Cl")==0 || strcmp(szElemName[i], "BR")==0 || strcmp(szElemName[i], "Br")==0 || strcmp(szElemName[i], "I")==0 )	{
			Is_F[i] = 1;
			GetBondList(i);
		}
...
void Output_Optimize_Water_Pose_QM(void)
{
	FILE *fOut;
	int i;
	char szName_Result[256]="opt-output.out", szDonor[256], szDonorH[256];
...
// TODO addapt this to fit any number of core (read it from input)
void RunAllJobs(void)
{
	int i, Idx_Worker, CoreNum, nJob_Submitted=0;
	int CoreNum_list8[]=  {8, 4, 4, 2, 4, 4, 4, 2, 2, 2, 2, 2, 2, 2, 2, 2};
	int CoreNum_List12[]={12, 6, 4, 3, 4, 2, 3, 3, 4, 4, 4, 2, 4, 4, 4, 3};
